...
func sendTaskToDeviceTask() {
	...
	key := fmt.Sprintf("agent/task/%s", common.ServConfig.AccessName)
...
func NewTaskToRedis(task *protos.Task) error {
	...
	// 同时更新一个队列
	err = common.RedisClient.LPush(context.Background(), fmt.Sprintf("agent/task/%s", task.AccessName), taskJson).Err()
	if err != nil {
