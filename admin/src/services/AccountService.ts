...
import type { HttpResponse, UserInfoStruct } from ".";

export class AccountService {
  static async userAuth() {
    const resp = await api.get<HttpResponse<UserInfoStruct>>("/usercenter", {
      headers: {
        "X-API": "user/auth",
      },
    });
  ...
  static async login(email: string, password: string, sms: string) {
    const resp = await api.post<HttpResponse<UserInfoStruct>>(
      "/usercenter",
      {
        cellphone: email,
        password: password,
  ...
  // 建账号
  static async create(obj: UserInfoStruct) {
