...
import {
  BotModel,
  BotService,
  RangeOscillationBotModel,
} from "@/services/BotService";
import { useMartParamStore } from "@/stores/martParam-store";
import { useI18n } from "vue-i18n";

defineOptions({
  name: "RangeOscillationRobotComponent",
});
const emits = defineEmits(["commited"]);

let model = ref<RangeOscillationBotModel>({
  totalCost: "",
  minPrice: "",
  maxPrice: "",
  minOrderSize: 0,
...
onMounted(async () => {
  ...
  if (resp && resp.code == 0) {
    if (resp.data && resp.data.rangeOscillationBot) {
      model.value = resp.data.rangeOscillationBot;
