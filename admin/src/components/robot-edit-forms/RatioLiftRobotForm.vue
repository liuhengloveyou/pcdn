...
defineOptions({
  name: "RatioLiftRobotComponent",
});
const emits = defineEmits(["commited"]);

let model = ref<RatioLiftBotModel>({
  targetPrice: "",
  totalCost: "",
  totalTime: 0,
  minOrderSize: 0,
...
const formSchema = toTypedSchema(
  z.object({
    ...
    totalTime: z.number().min(10),
    minOrderSize: z.number().min(0),
    maxOrderSize: z.number().min(0),
    minOscillate: z.number().min(0),
    maxOscillate: z.number().min(0),
    oscillateRatio: z.number().min(0),
...
onMounted(async () => {
  ...
  if (resp && resp.code == 0) {
    if (resp.data && resp.data.ratioLiftBot) {
      model.value = resp.data.ratioLiftBot;
