...
defineOptions({
  name: "BalanceMonitorRobotComponent",
});
...
let model = ref<BalanceMonitorRobotModel>({
  id: 0,
  mail1: "",
  mail2: "",
  mail3: "",
...
onMounted(async () => {
  ...
  if (resp) {
    console.log(">>>", resp);
    if (resp.code == 0) {
      if (resp.data && resp.data.balanceMonitorBot) {
        model.value = resp.data.balanceMonitorBot;
