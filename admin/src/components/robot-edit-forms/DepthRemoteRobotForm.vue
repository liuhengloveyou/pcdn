...
defineOptions({
  name: "DepthRemoteRobotComponent",
});
...
let model = ref<DepthRemoteBotModel>({
  id: 0,
  minBuyOrderSize: 0,
  maxBuyOrderSize: 0,
  minBuySheet: 0,
...
const formSchema = toTypedSchema(
  z.object({
    minBuyOrderSize: z.number().min(1).max(10000000),
    maxBuyOrderSize: z.number().min(1).max(10000000),
    minBuySheet: z.number().min(1).max(1000),
    maxBuySheet: z.number().min(1).max(1000),
    minBuySpread: z.number().min(1).max(100),
    maxBuySpread: z.number().min(1).max(100),
...
onMounted(async () => {
  ...
  if (resp) {
    if (resp.code == 0) {
      if (resp.data && resp.data.depthRemoteBot) {
        model.value = resp.data.depthRemoteBot;
    ...
    <div class="space-y-2">
      ...
      <div class="flex row justify-between">
        <FormField v-slot="{ componentField }" name="minBuyOrderSize">
          <FormItem>
            <NumberField class="gap-2" v-bind="componentField">
              <NumberFieldContent>
                <NumberFieldDecrement />
                <FormControl>
                  <NumberFieldInput />
