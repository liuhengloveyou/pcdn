...
let model = ref<TGScheduledReportRobotModel>({
  id: 0,
  chatId: "",
  userName: "",
  remindInterval: 0,
...
onMounted(async () => {
  ...
  if (resp) {
    console.log(">>>", resp);
    if (resp.code == 0) {
      if (resp.data && resp.data.tgScheduledReportBot) {
        model.value = resp.data.tgScheduledReportBot;
