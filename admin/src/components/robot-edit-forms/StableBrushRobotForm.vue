...
defineOptions({
  name: "EditStableBrushRobotComponent",
});
...
let model = ref<StableBrushBotModel>({
  id: 0,
  // 单笔交易量：
  minNumPeerOrder: 10,
  maxNumPeerOrder: 100,
...
onMounted(async () => {
  ...
  if (resp) {
    if (resp.code == 0) {
      if (resp.data && resp.data.stableBrushBot) {
        model.value = resp.data.stableBrushBot;
  ...
  console.log("stableBrushBot>>>bot: ", model);
});
