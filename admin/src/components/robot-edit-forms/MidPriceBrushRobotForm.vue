...
defineOptions({
  name: "EditNaturalBrushRobotComponent",
});
...
let model = ref<NaturalBrushRobotModel>({
  id: 0,
  // 单笔交易量：
  minNumPeerOrder: 10,
  maxNumPeerOrder: 100,
...
onMounted(async () => {
  ...
  if (resp) {
    if (resp.code == 0) {
      if (resp.data && resp.data.naturalBrushBot) {
        model.value = resp.data.naturalBrushBot;
  ...
  console.log("EditNaturalBrushRobotComponent>>>bot: ", model);
});
...
<template>
  ...
  <Toaster class="bg-primary dark:bg-primary" />
</template>
