...
import { BotModel, BotService, RatioDownBotModel } from "@/services/BotService";
import { useMartParamStore } from "@/stores/martParam-store";
import { useI18n } from "vue-i18n";

defineOptions({
  name: "RatioDownRobotComponent",
});
const emits = defineEmits(["commited"]);

let model = ref<RatioDownBotModel>({
  targetPrice: "",
  totalCost: "",
  totalTime: 1,
  minOrderSize: 0,
...
const formSchema = toTypedSchema(
  z.object({
    ...
    minOscillate: z.number().min(0),
    maxOscillate: z.number().min(1),
    oscillateRatio: z.number().min(1),
...
onMounted(async () => {
  ...
  if (resp && resp.code == 0) {
    if (resp.data && resp.data.ratioDownBot) {
      model.value = resp.data.ratioDownBot;
      ...
      setFieldValue("oscillateRatio", model.value.oscillateRatio);
...
async function onRealSubmit(m: GenericObject) {
  ...
  if (
    ...
    toast({
      variant: "destructive",
      title: "please input <Oscillate Range>.",
    });
    return;
  }

  data.oscillateRatio = Number(m.oscillateRatio);
  if (Number.isNaN(data.oscillateRatio) || data.oscillateRatio <= 0) {
    toast({
      variant: "destructive",
      title: "please input <Oscillate Ratio>.",
    });
    return;
  }
...
<template>
  <form class="space-y-6" @submit="onSubmit">
    ...
    <FormField v-slot="{ componentField }" name="oscillateRatio">
      <FormItem>
        <FormLabel>{{ $t("oscillateRatio") }}(%):</FormLabel>
        <FormControl>
          <Input type="number" v-bind="componentField" />
        </FormControl>
        <FormMessage />
